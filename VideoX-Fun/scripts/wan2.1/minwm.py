"""Camera-controlled bidirectional diffusion trainer.

Standalone trainer that swaps:
1. Model: CameraBidirectionalDiffusion (use_camera=True)
...
class Trainer():
    def __init__(self, config):
        # Replicate parent __init__ but swap model and dataset
        self.config = config
        self.step = 0
        ...
        from model import CameraBidirectionalDiffusion
        from pipeline import BidirectionalDiffusionInferencePipeline
        self._BidirectionalDiffusionInferencePipeline = BidirectionalDiffusionInferencePipeline
        ...
        if self.eval_interval:
            self.pipeline = self._BidirectionalDiffusionInferencePipeline(config, device=self.device)
    ...
    def train_one_step(self, batch):
        ...
        if self.is_main_process and not getattr(self, "_first_bp_logged", False):
            print("[Trainer Entry] Wan21/wan_trainer/camera_bidirectional_diffusion.py :: Trainer.train_one_step (first BP done)", flush=True)
