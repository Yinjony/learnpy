...
def main():
    ...
    #lyz: Unfreeze score conditioning modules
    if hasattr(transformer3d, 'quality_embedding'):
        print("find quality_embedding")
        transformer3d.quality_embedding.requires_grad_(True)
    if hasattr(transformer3d, 'quality_projection'):
        print("find quality_projection")
        transformer3d.quality_projection.requires_grad_(True)
    if hasattr(transformer3d, 'motion_embedding'):
        print("find motion_embedding")
        transformer3d.motion_embedding.requires_grad_(True)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                #lyz: we have ortho_loss here
                W_q = accelerator.unwrap_model(transformer3d).quality_projection[-1].weight
                W_m = accelerator.unwrap_model(transformer3d).motion_projection[-1].weight
                ortho_loss = (W_q.T @ W_m).pow(2).mean()
...
#
                ...
                #            if any(k in name for k in ['quality_', 'motion_']):
                #                print(f"[Step {global_step}] {name}: "
                #                    f"grad_norm={param.grad.norm().item():.6f}, "
                #                    f"param_std={param.data.std().item():.6f}, "
                #                    f"param_max={param.data.abs().max().item():.6f}")
#
                #    # lyz: 重点看这两个：projection最后一层应该从零开始长出非零值
                #    for key in ['quality_projection.1', 'motion_projection.1']:
                #        for name, param in model.named_parameters():
                #            if key in name and 'weight' in name:
                #                print(f"[Step {global_step}] {name}: "
                #                    f"std={param.data.std().item():.8f} "
                #                    f"(should grow from 0)")
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
            ...
            logs = {"step_loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0], "ortho": ortho_loss.detach().item()}
