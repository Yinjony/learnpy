...
def main():
    ...
    #lyz: ====== θ₁: Stage1 expert model (frozen, with score ability) ======
    transformer_theta1 = WanTransformer3DModelOUR.from_pretrained(
        os.path.join(args.pretrained_model_name_or_path,
    ...
    fake_pipe = FakePipeline(transformer_theta1)
    fake_pipe = merge_lora(fake_pipe, args.stage1_lora_path, 1.0,
                           device=accelerator.device, dtype=weight_dtype)
    transformer_theta1 = fake_pipe.transformer

    # 加载Stage1 score modules
    from safetensors.torch import load_file
    score_state_dict = load_file(args.stage1_score_path)
    m, u = transformer_theta1.load_state_dict(score_state_dict, strict=False)
    print(f"theta1 score modules loaded — missing: {len(m)}, unexpected: {len(u)}")

    transformer_theta1.requires_grad_(False)
    transformer_theta1.eval()

    # lyz: ====== θ / θ_ref: target model (new LoRA to train, NO score) ======
                ...
                if os.path.exists(pkl_path):
                    with open(pkl_path, 'rb') as file:
                        ...
                        batch_sampler.sampler._pos_start = max(
                            loaded_number - args.dataloader_num_workers * accelerator.num_processes * 2, 0)
    ...
    if args.enable_bucket:
        aspect_ratio_sample_size = {key: [x / 512 * args.video_sample_size for x in ASPECT_RATIO_512[key]] for key in
        ...
        def get_length_to_frame_num(token_length):
            ...
            length_to_frame_num = {
                sample_size: min(token_length / sample_size / sample_size,
                                 args.video_sample_n_frames) // sample_n_frames_bucket_interval * sample_n_frames_bucket_interval + 1
        ...
        def collate_fn(examples):
            ...
            if data_type == 'image':
                ...
                aspect_ratio_sample_size = {
                    key: [x / 512 * args.image_sample_size / random_downsample_ratio for x in ASPECT_RATIO_512[key]] for
                    key in ASPECT_RATIO_512.keys()}
                aspect_ratio_random_crop_sample_size = {
                    key: [x / 512 * args.image_sample_size / random_downsample_ratio for x in
                          ASPECT_RATIO_RANDOM_CROP_512[key]] for key in ASPECT_RATIO_RANDOM_CROP_512.keys()}
                ...
                aspect_ratio_sample_size = {
                    key: [x / 512 * args.video_sample_size / random_downsample_ratio for x in ASPECT_RATIO_512[key]] for
                    key in ASPECT_RATIO_512.keys()}
                aspect_ratio_random_crop_sample_size = {
                    key: [x / 512 * args.video_sample_size / random_downsample_ratio for x in
                          ASPECT_RATIO_RANDOM_CROP_512[key]] for key in ASPECT_RATIO_RANDOM_CROP_512.keys()}
            ...
            batch_video_length = (
                                             batch_video_length - 1) // sample_n_frames_bucket_interval * sample_n_frames_bucket_interval + 1
            ...
            for i in range(bsz):
                r = np.random.rand()
                if r < 0.05:
                    new_examples["quality_score"][i] = 0.0
                    new_examples["motion_score"][i] = 0.0
                elif r < 0.15:
                    new_examples["quality_score"][i] = 0.0
                elif r < 0.25:
                    new_examples["motion_score"][i] = 0.0
            ...
            if args.train_mode != "normal":
                new_examples["mask_pixel_values"] = torch.stack(
                    [example for example in new_examples["mask_pixel_values"]])
        ...
        # DataLoaders creation:
        batch_sampler_generator = torch.Generator().manual_seed(args.seed)
        batch_sampler = ImageVideoSampler(RandomSampler(train_dataset, generator=batch_sampler_generator),
    ...
    vae.to(accelerator.device if not args.low_vram else "cpu", dtype=weight_dtype)
    transformer3d.to(accelerator.device, dtype=weight_dtype)
    transformer_theta1.to(accelerator.device, dtype=weight_dtype) #lyz: add to gpu
        ...
        for step, batch in enumerate(train_dataloader):
            if epoch == first_epoch and step == 0:
                ...
                for idx, (pixel_value, text) in enumerate(zip(pixel_values, texts)):
                    ...
                    gif_name = '-'.join(
                        text.replace('/', '').split()[:10]) if not text == '' else f'{global_step}-{idx}'
                    save_videos_grid(pixel_value, f"{args.output_dir}/sanity_check/{gif_name[:10]}.gif", rescale=True)
                if args.train_mode != "normal":
                    clip_pixel_values, mask_pixel_values, texts = batch['clip_pixel_values'].cpu(), batch[
                        'mask_pixel_values'].cpu(), batch['text']
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if args.random_frame_crop:
                    ...
                    select_frames = [_tmp for _tmp in list(range(sample_n_frames_bucket_interval + 1,
                ...
                if args.keep_all_node_same_token_length:
                    ...
                    actual_video_length = (min(
                        actual_token_length / pixel_values.size()[-1] / pixel_values.size()[-2],
                ...
                with torch.no_grad():
                    ...
                    if args.train_mode != "normal":
                        ...
                        for clip_pixel_value in clip_pixel_values:
                            ...
                            clip_image = TF.to_tensor(clip_image).sub_(0.5).div_(0.5).to(clip_image_encoder.device,
                                                                                         weight_dtype)
                ...
                if args.enable_text_encoder_in_dataloader:
                    ...
                    with torch.no_grad():
                        ...
                        prompt_embeds = text_encoder(text_input_ids.to(latents.device),
                                                     attention_mask=prompt_attention_mask.to(latents.device))[0]
...
#
                ...
                with torch.no_grad(), torch.cuda.amp.autocast(dtype=weight_dtype):
                    ...
                    #    x=noisy_latents, context=prompt_embeds, t=timesteps,
                    #    seq_len=seq_len, quality_score=quality_scores, motion_score=motion_scores)
                    ...
                    delta_q = delta_q / (delta_q_flat.norm(dim=1, keepdim=True).view(-1, 1, 1, 1, 1) + 1e-8)
                    delta_m_flat = delta_m.flatten(1)
                    delta_m = delta_m / (delta_m_flat.norm(dim=1, keepdim=True).view(-1, 1, 1, 1, 1) + 1e-8)

                    # 构造winner/loser（从样本真实水平出发）
                    z_w = pred_null + args.lambda_q * delta_q + args.lambda_m * delta_m
                    z_l = pred_null - args.lambda_q * delta_q - args.lambda_m * delta_m
                ...
                noise_pred_ref_f = noise_pred_ref.float()
                z_w_f = z_w.float()
                z_l_f = z_l.float()
