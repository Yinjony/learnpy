...
export WORLD_MODEL_MODULES="/ltl/VideoX-Fun/output_dir_cpo_stage1/score_modules-5000.safetensors"

accelerate launch --mixed_precision="bf16" train_grpo_world_ours.py \
  ...
  --learning_rate=1e-05 \
  --seed=42 \
  --output_dir="output_dir_grpo_world_1.3b" \
  ...
  --use_peft_lora \
  --enable_world_model \
  --world_model_modules_path=$WORLD_MODEL_MODULES \
  --history_video_column="original_video" \
