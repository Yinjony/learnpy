...
VIDEOX_FUN_ROOT = Path(__file__).resolve().parents[3]


def get_wan_model_dir(model_name="Wan2.1-T2V-1.3B"):
    override = os.environ.get("WAN_MODEL_DIR")
    if override:
        return Path(override).expanduser().resolve()
    return VIDEOX_FUN_ROOT / "models" / model_name
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        ...
        self.model = _video_vae(
            pretrained_path=str(get_wan_model_dir() / "Wan2.1_VAE.pth"),
            z_dim=16,
        ).eval().requires_grad_(False)
    ...
    def __init__(
        ...
        if is_causal:
            ...
            self.model = CausalWanModel.from_pretrained(
                str(get_wan_model_dir(model_name)), local_attn_size=local_attn_size, sink_size=sink_size)
        else:
            self.model = WanModel.from_pretrained(str(get_wan_model_dir(model_name)))
        self.model.eval()
