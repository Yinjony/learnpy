...
class ImageVideoDataset(Dataset):
    def __init__(
        ...
        # === New: store score column names, normalize to list ===
        if quality_score_columns is None:
            self.quality_score_columns = []
        elif isinstance(quality_score_columns, str):
            self.quality_score_columns = [quality_score_columns]
        else:
            self.quality_score_columns = list(quality_score_columns)

        if motion_score_columns is None:
            self.motion_score_columns = []
        elif isinstance(motion_score_columns, str):
            self.motion_score_columns = [motion_score_columns]
        else:
            self.motion_score_columns = list(motion_score_columns)
        ...
        while True:
            ...
            try:
                ...
                if len(self.quality_score_columns) > 0:
                    q_values = [float(data_info_local.get(col, 0.0)) for col in self.quality_score_columns]
                    sample["quality_score"] = sum(q_values) / len(q_values)
                else:
                    sample["quality_score"] = 0.0

                if len(self.motion_score_columns) > 0:
                    m_values = [float(data_info_local.get(col, 0.0)) for col in self.motion_score_columns]
                    sample["motion_score"] = sum(m_values) / len(m_values)
                else:
                    sample["motion_score"] = 0.0
