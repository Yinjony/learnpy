...
from .wan_prope import expand_camera_matrices, prope_qkv
...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, dtype=torch.bfloat16, t=0, viewmats=None, Ks=None):
        r"""
        Args:
        ...
        if prope_enabled:
            ...
            x = x + self.prope_o(x_prope.flatten(2).to(dtype))
        return x
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        t=0,
        quality_e0=None, motion_e0=None, #lyz: sign embeddings
        ...
        # lyz: deal with quality_e0/motion_e0
        if quality_e0 is not None:
            if quality_e0.dim() > 3:
                q = quality_e0.chunk(4, dim=2)
                q = [qi.squeeze(2) for qi in q]
            else:
                q = quality_e0.chunk(4, dim=1)
        else:
            q = None

        if motion_e0 is not None:
            if motion_e0.dim() > 3:
                m = motion_e0.chunk(4, dim=2)
                m = [mi.squeeze(2) for mi in m]
            else:
                m = motion_e0.chunk(4, dim=1)
        else:
            m = None

        #print("quality and motion e0:", quality_e0, motion_e0)
        #print("q and m:", q, m)

        # self-attention
        #temp_x = self.norm1(x) * (1 + e[1]) + e[0]
        ##temp_x = self.norm1(x) * (1 + e[1] + (q[1] if q is not None else 0) + (m[1] if m is not None else 0)) \
        ##         + e[0] + (q[0] if q is not None else 0) + (m[0] if m is not None else 0)
#
        ## lyz:Quality modulation (multiplicative scale, additive shift)
        #if q is not None:
        #    temp_x = temp_x * (1 + q[1]) + q[0]
        ##lyz:Motion modulation (multiplicative scale, additive shift)
        #if m is not None:
        #    temp_x = temp_x * (1 + m[1]) + m[0]

        norm1_x = self.norm1(x)
        temp_x = norm1_x * (1 + e[1]) + e[0]

        if q is not None:
            temp_x = temp_x + norm1_x * q[1] + q[0]

        if m is not None:
            temp_x = temp_x + norm1_x * m[1] + m[0]
#
        temp_x = temp_x.to(dtype)



        y = self.self_attn(temp_x, seq_lens, grid_sizes, freqs, dtype, t=t, viewmats=viewmats, Ks=Ks)
        x = x + y * e[2]

        # cross-attention & ffn function
        def cross_attn_ffn(x, context, context_lens, e, q, m): #lyz: add q m
            # cross-attention
            x = x + self.cross_attn(self.norm3(x), context, context_lens, dtype, t=t)

            # ffn function
            #temp_x = self.norm2(x) * (1 + e[4] + (q[3] if q is not None else 0) + (m[3] if m is not None else 0)) \
            #         + e[3] + (q[2] if q is not None else 0) + (m[2] if m is not None else 0)

            #temp_x = self.norm2(x) * (1 + e[4]) + e[3]
#
            #if q is not None:
            #    temp_x = temp_x * (1 + q[3]) + q[2]   # quality
            #if m is not None:
            #    temp_x = temp_x * (1 + m[3]) + m[2]   # motion

            norm2_x = self.norm2(x)
            temp_x = norm2_x * (1 + e[4]) + e[3]
            if q is not None:
                temp_x = temp_x + norm2_x * q[3] + q[2]
            if m is not None:
                temp_x = temp_x + norm2_x * m[3] + m[2]
        ...
        x = cross_attn_ffn(x, context, context_lens, e, q, m)
        return x
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        #lyz: mlp for tokens
        self.quality_embedding  = nn.Sequential(nn.Linear(freq_dim, dim), nn.SiLU(), nn.Linear(dim, dim))
        self.quality_projection = nn.Sequential(nn.SiLU(), nn.Linear(dim, dim * 4)) #lyz: we don need gate

        self.motion_embedding   = nn.Sequential(nn.Linear(freq_dim, dim), nn.SiLU(), nn.Linear(dim, dim))
        self.motion_projection  = nn.Sequential(nn.SiLU(), nn.Linear(dim, dim * 4)) #lyz: we don need gate
    ...
    def forward(
        ...
        subject_ref=None,
        cond_flag=True,
        quality_score=None, motion_score=None #lyz: sign tokens
    ):
        r"""
        ...
        if viewmats is not None and (full_ref is not None or subject_ref is not None):
            raise ValueError("PRoPE world-model training does not support reference tokens.")
        ...
        if viewmats is not None:
            if Ks is None:
                raise ValueError("Ks must be provided with viewmats for PRoPE world-model training.")
            viewmats, Ks = expand_camera_matrices(viewmats, Ks, grid_sizes, seq_len)
            viewmats = viewmats.to(device=x.device)
            Ks = Ks.to(device=x.device)
        ...
        with amp.autocast(dtype=torch.float32):
            if t.dim() != 1:
                ...
                # lyz: Score: 始终是标量，算出 [B, 6, dim] 再 unsqueeze 到 [B, 1, 6, dim] 广播
                quality_e0 = None
                if quality_score is not None:
                    q_e = self.quality_embedding(
                        sinusoidal_embedding_1d(self.freq_dim, quality_score).float())
                    quality_e0 = self.quality_projection(q_e).unflatten(1, (4, self.dim))  # [B, 4, dim]
                    quality_e0 = quality_e0.unsqueeze(1)  # [B, 1, 4, dim]
                    q_mask = (quality_score != 0.0).float().view(-1, *([1] * (quality_e0.dim() - 1)))
                    quality_e0 = quality_e0 * q_mask


                motion_e0 = None
                if motion_score is not None:
                    m_e = self.motion_embedding(
                        sinusoidal_embedding_1d(self.freq_dim, motion_score).float())
                    motion_e0 = self.motion_projection(m_e).unflatten(1, (4, self.dim))
                    motion_e0 = motion_e0.unsqueeze(1)  # [B, 1, 4, dim]
                    m_mask = (motion_score != 0.0).float().view(-1, *([1] * (motion_e0.dim() - 1)))
                    motion_e0 = motion_e0 * m_mask
                ...
                # Score: 同样 [B, 6, dim]，不需要unsqueeze
                quality_e0 = None
                if quality_score is not None:
                    q_e = self.quality_embedding(
                        sinusoidal_embedding_1d(self.freq_dim, quality_score).float())
                    quality_e0 = self.quality_projection(q_e).unflatten(1, (4, self.dim))
                    q_mask = (quality_score != 0.0).float().view(-1, *([1] * (quality_e0.dim() - 1)))
                    quality_e0 = quality_e0 * q_mask
        ...
        if self.sp_world_size > 1:
            ...
            if viewmats is not None:
                viewmats = torch.chunk(viewmats, self.sp_world_size, dim=1)[self.sp_world_rank]
                Ks = torch.chunk(Ks, self.sp_world_size, dim=1)[self.sp_world_rank]
            if t.dim() != 1:
                ...
                for block in self.blocks:
                    if torch.is_grad_enabled() and self.gradient_checkpointing:
                        ...
                        x = torch.utils.checkpoint.checkpoint(
                            ...
                            dtype,
                            t,
                            quality_e0,  # lyz: we add
                            motion_e0,  # lyz: we add
                            viewmats,
                            Ks,
                            **ckpt_kwargs,
                        )
                        ...
                        kwargs = dict(
                            ...
                            dtype=dtype,
                            t=t,
                            quality_e0=quality_e0,  # lyz: we add
                            motion_e0=motion_e0,  # lyz: we add
                            viewmats=viewmats,
                            Ks=Ks,
                        )
                        x = block(x, **kwargs)
    ...
    def init_weights(self):
        ...
        #lyz: we add init
        nn.init.zeros_(self.quality_projection[-1].weight)
        nn.init.zeros_(self.quality_projection[-1].bias)
