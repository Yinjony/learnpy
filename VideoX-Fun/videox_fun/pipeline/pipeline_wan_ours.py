...
class WanPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        shift: int = 5,
        quality_score: Optional[float] = None, #lyz:add score input
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    q_scores = torch.tensor([0.0] * batch_size + [quality_score] * batch_size, device=device,
                                            dtype=weight_dtype) if quality_score is not None else None
                    m_scores = torch.tensor([0.0] * batch_size + [motion_score] * batch_size, device=device,
                                            dtype=weight_dtype) if motion_score is not None else None
                else:
                    q_scores = torch.tensor([quality_score] * batch_size, device=device,
                                            dtype=weight_dtype) if quality_score is not None else None
                    m_scores = torch.tensor([motion_score] * batch_size, device=device,
                                            dtype=weight_dtype) if motion_score is not None else None
